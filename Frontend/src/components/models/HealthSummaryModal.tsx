...
const tableData = [
  ...
  { metric: "Daily Steps", avg: "8,240", baseline: "8,000", status: "Optimal" },
  {
    metric: "Skin Temp",
    avg: "36.5°C",
    baseline: "36.1-37.2°C",
    status: "Normal",
  },
  {
    metric: "Respiratory Rate",
    avg: "16 br/min",
    baseline: "12-20 br/min",
    status: "Normal",
  },
];
...
const HealthSummaryModal = ({
  open,
  onOpenChange,
}: HealthSummaryModalProps) => {
  const [selectedDuration, setSelectedDuration] = useState("7 Days");
  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false);
  const emrDocumentRef = useRef<HTMLDivElement>(null);

  const handleDownloadPDF = async () => {
    if (!emrDocumentRef.current) return;

    setIsGeneratingPDF(true);
    try {
      // Capture the EMR document as a canvas
      const canvas = await html2canvas(emrDocumentRef.current, {
      ...
      const fileName = `Healix_Clinical_EMR_${new Date().toISOString().split("T")[0]}.pdf`;
      pdf.save(fileName);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg border-slate-200 bg-[#FAFAFA] overflow-y-auto h-[36rem] sm:max-w-2xl p-0">
        ...
        <div
          ref={emrDocumentRef}
          className="m-6 mt-2 rounded-xl border border-slate-200 bg-white p-6 shadow-sm"
        >
          {/* Letterhead */}
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            ...
            <span className="text-[10px] text-slate-400">
              Generated: {today} | Auth: System AI
            </span>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-3 rounded-xl bg-indigo-50/50 p-4">
              ...
              <p className="text-sm leading-relaxed text-slate-600">
                Based on the continuous telemetry data collected over the past{" "}
                {selectedDuration.toLowerCase()}, the patient is maintaining
                optimal cardiovascular health. Resting heart rate has decreased
                by 3% week-over-week. Dietary adherence to the low-sodium
                protocol is currently at 85%. No emergency arrhythmias have been
                detected.
              </p>
            </div>
          </div>
            ...
            <thead>
              <tr className="border-b border-slate-100 text-xs uppercase tracking-widest text-slate-400">
                ...
                <th className="pb-2 font-medium">
                  {selectedDuration.replace(" Days", "d")} Avg
                </th>
        ...
        <div className="flex gap-3 border-t border-slate-100 px-6 py-4">
          <Button
            variant="outline"
            className="flex-1 rounded-xl border-slate-200 text-sm text-slate-600 hover:bg-slate-50"
          >
          ...
          <Button
            ...
            className="flex-1 rounded-xl bg-indigo-600 text-sm hover:bg-indigo-700 disabled:opacity-50"
          >
